...
class ReportController extends Controller
{
    ...
    public function arrears(Request $request)
    {
        ...
        if ($childName) {
            $children->where('name', 'like', '%' . $childName . '%');
        }
        $children = $children->with(['invoicePayments', 'therapyTypes', 'vocationalTypes'])->get();

        foreach ($children as $child) {
            $child->paymentStatus = $this->getPaymentStatus($child, $month, $year);
            $child->invoiceAmount = $child->calculateInvoiceAmount($month, $year);

            $incomeQuery = $child->incomes()->whereMonth('date', $month)->whereYear('date', $year);
            if ($walletId) {
                $incomeQuery->where('wallet_id', $walletId);
            }
            $child->totalPaid = $incomeQuery->sum('amount');
            $child->outstanding = max(0, $child->invoiceAmount - $child->totalPaid);
        }

        if ($status) {
            $children = $children->filter(function ($child) use ($status) {
                return $child->paymentStatus === $status;
            });
        }

        $summary = [
            'paid' => $children->where('paymentStatus', 'paid')->count(),
            'partial' => $children->where('paymentStatus', 'partial')->count(),
            'unpaid' => $children->where('paymentStatus', 'unpaid')->count(),
            'overdue' => $children->where('paymentStatus', 'overdue')->count(),
            'totalOutstanding' => $children->sum('outstanding'),
        ];

        $wallets = Wallet::all(['id', 'name']);
        $selectedWallet = $walletId ? $wallets->firstWhere('id', $walletId) : null;

        return view('reports.arrears', compact('children', 'summary', 'year', 'month', 'status', 'wallets', 'selectedWallet', 'walletId'));
    }
    ...
    public function exportArrearsPdf(Request $request)
    {
        ...
        ActivityLog::create([
            ...
            'description' => auth()->user()->name . ' mengekspor PDF Rekap Tunggakan (' . Carbon::create($year, $month, 1)->locale('id')->format('F Y') . ')',
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
        ]);

        $pdf = Pdf::loadView('reports.pdf.arrears', [
            'children' => $children,
            'year' => $year,
            'month' => $month,
            'status' => $status,
        ...
        return $pdf->download('Laporan-Tunggakan-' . date('Y-m') . '.pdf');
    }

    public function exportArrearsExcel(Request $request)
    {
        $year = $request->input('year', now()->year);
        $month = $request->input('month', now()->month);
        $status = $request->input('status');
        ...
        ActivityLog::create([
            ...
            'action' => 'export_excel',
            'subject_type' => Child::class,
            'description' => auth()->user()->name . ' mengekspor Excel Rekap Tunggakan (' . Carbon::create($year, $month, 1)->locale('id')->format('F Y') . ')',
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
        ]);

        $headers = ['nama_anak', 'layanan', 'tagihan', 'sudah_dibayar', 'tunggakan', 'status'];
        ...
        return Excel::download(new ArrearsExport($childrenCollection, $period), 'Laporan-Tunggakan-' . date('Y-m') . '.xlsx');
    }
