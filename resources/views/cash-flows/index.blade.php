        ...
        <div class="relative z-10 px-6 py-5">
            <div class="flex items-center gap-5">
                ...
                <div class="text-right flex-shrink-0">
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Saldo Awal</p>
                    <p class="text-xl sm:text-2xl font-bold text-gray-600">
                        Rp {{ number_format($openingBalance ?? 0, 0, ',', '.') }}
                    </p>
                    <p class="text-xs text-gray-400 mt-1">Saldo sebelum periode ini</p>
                    <div class="mt-2 pt-2 border-t border-gray-200/60">
                        <p class="text-xs text-gray-400 font-medium uppercase tracking-wider">Saldo Tersedia</p>
            ...
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                ...
                <div class="w-[170px]">
                    ...
                    <select name="month" onchange="this.form.submit()"
                        class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white/60 appearance-none cursor-pointer">
                        <option value="">— Semua Bulan —</option>
                        @foreach($monthOptions as $key => $label)
                            <option value="{{ $key }}" {{ request('month') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="w-[150px]">
                ...
                @if(request('search') || request('wallet_id') || request('month') || request('date_from') || request('date_to'))
                <form method="GET" action="{{ route('cash-flows') }}" class="inline">
                    @if(request('wallet_id'))
        ...
        <form action="{{ route('incomes.store') }}" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <select name="income_category_id" id="incomeCategoryId" required
                        data-categories='@json($incomeCategories->map(fn($c) => ['name' => $c->name, 'id' => $c->id]))'
