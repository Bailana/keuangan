...
@section('page-title', 'Tambah Anak')
...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900">Tambah Anak</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 sm:p-8">
        <form action="{{ route('children.store') }}" method="POST" class="space-y-6" id="childForm">
            @csrf
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-200" data-therapy-id="{{ $therapy->id }}" data-therapy-price="{{ $therapy->price_per_session }}">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="number" name="therapy_sessions[{{ $therapy->id }}]" id="therapy_{{ $therapy->id }}"
                                min="0" max="30" value="0" placeholder="0"
            ...
            <div>
                ...
                <div class="p-3 bg-blue-50 rounded-xl border border-blue-200">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_sekolah" value="1" {{ old('is_sekolah') ? 'checked' : '' }}
                            class="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                            onchange="toggleSekolah(this)">
                        <span class="font-medium text-gray-900">Aktif mengikuti sekolah</span>
                    </label>
                    <div id="schoolClassField" class="mt-3 {{ old('is_sekolah') ? '' : 'hidden' }}">
                        ...
                        <input type="number" name="spp_fee" id="spp_fee" value="{{ old('spp_fee', config('settings.school_fee', 1000000)) }}" min="0"
                            class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 px-4 py-2.5 text-sm transition-all"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="subsidi_amount" id="subsidi_amount"
                                    min="0" step="100" value="{{ old('subsidi_amount') }}"
            ...
            <div>
                ...
                <div class="p-4 bg-sky-50 rounded-xl border border-sky-200">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="has_parent_support" value="1" id="has_parent_support" {{ old('has_parent_support') ? 'checked' : '' }}
            ...
            <div id="estimateCard" class="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4" style="display: block;">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-2xl font-bold text-emerald-700 mt-1" id="estimateAmount">Rp 0</div>
                    </div>
...
function calculateEstimate() {
    ...
    if (sekolahCheck) {
        ...
        const sppPrice = sppInput ? (parseFloat(sppInput.value) || {{ config('settings.school_fee', 1000000) }}) : 0;
        total += sppPrice;
