...
<div class="space-y-6 relative">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($paginatedChildren as $child)
                ...
                <div class="mt-auto flex items-center justify-between pt-4 border-t border-white/50">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @if($child->payment_status === 'paid')
                            <a href="{{ route('invoices.generatePaid', ['child' => $child->id, 'month' => $currentMonth, 'year' => $currentYear]) . '?' . http_build_query(['page' => request('page', 1)]) }}"
                               target="_blank"
                        ...
                        @else
                            <a href="{{ route('invoices.generate', ['child' => $child->id, 'month' => $currentMonth, 'year' => $currentYear]) . '?' . http_build_query(['page' => request('page', 1)]) }}"
                               target="_blank"
                        ...
                        <!-- WhatsApp -->
                        <a href="{{ route('invoices.whatsapp', ['child' => $child->id, 'month' => $currentMonth, 'year' => $currentYear]) . '?' . http_build_query(['page' => request('page', 1)]) }}"
                           target="_blank"
    ...
    <!-- Pagination -->
    @if($paginatedChildren && $paginatedChildren->hasPages())
    <div class="mt-6 flex items-center justify-between gap-2">
        <p class="text-xs text-gray-400 shrink-0">
            Halaman {{ $paginatedChildren->currentPage() }} dari {{ $paginatedChildren->lastPage() }} — {{ $paginatedChildren->total() }} invoice
        </p>
        <nav class="flex items-center gap-1 shrink-0" aria-label="Pagination">
            {{-- Previous --}}
            @if ($paginatedChildren->onFirstPage())
                <span class="px-2.5 py-1.5 rounded-lg text-xs text-gray-300 cursor-not-allowed opacity-50">Prev</span>
            @else
                <a href="{{ $paginatedChildren->previousPageUrl() }}" class="px-2.5 py-1.5 rounded-lg text-xs font-medium text-gray-600 hover:bg-white/60 hover:text-gray-900 transition-colors border border-gray-200">Prev</a>
            @endif

            {{-- Pages --}}
            @foreach ($paginatedChildren->getUrlRange(1, $paginatedChildren->lastPage()) as $page => $url)
                @if ($page == $paginatedChildren->currentPage())
            ...
            {{-- Next --}}
            @if ($paginatedChildren->hasMorePages())
                <a href="{{ $paginatedChildren->nextPageUrl() }}" class="px-2.5 py-1.5 rounded-lg text-xs font-medium text-gray-600 hover:bg-white/60 hover:text-gray-900 transition-colors border border-gray-200">Next</a>
            @else
