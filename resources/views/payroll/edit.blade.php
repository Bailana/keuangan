...
<div class="space-y-6 relative">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Gaji Karyawan</h1>
            <p class="text-xs sm:text-sm text-gray-500 mt-1">{{ $payroll->employee_name }} — {{ \Carbon\Carbon::create($payroll->year, $payroll->month, 1)->locale('id')->format('F Y') }}</p>
        </div>
    </div>
            ...
            <form action="{{ route('payroll.update', $payroll) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="employee_name" id="employeeName" required
                            ...
                            @foreach($employees as $emp)
                                <option value="{{ $emp->name }}" {{ old('employee_name', $payroll->employee_name) == $emp->name ? 'selected' : '' }}
                                    data-position="{{ $emp->position }}" data-whatsapp="{{ $emp->whatsapp }}" data-phone="{{ $emp->phone }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="position" id="position" value="{{ old('position', $payroll->position) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $payroll->phone) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">WhatsApp</label>
                        <input type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $payroll->whatsapp) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="month" required class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all bg-white">
                            @foreach(range(1, 12) as $m)
                                <option value="{{ $m }}" {{ old('month', $payroll->month) == $m ? 'selected' : '' }}>
                                    {{ \Carbon\Carbon::create($payroll->year, $m, 1)->locale('id')->format('F') }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="number" name="year" required min="2020" max="2030" value="{{ old('year', $payroll->year) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Tanggal Payroll</label>
                        <input type="date" name="salary_date" value="{{ old('salary_date', $payroll->salary_date?->format('Y-m-d')) }}" required
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="base_salary" min="0" step="1000" id="baseSalary"
                            value="{{ old('base_salary', (int)($payroll->base_salary ?? 0)) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Tunjangan Lain (Rp)</label>
                        <input type="number" name="salary_extra" min="0" step="1000" id="salaryExtra"
                            value="{{ old('salary_extra', (int)($payroll->salary_extra ?? 0)) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Total Sesi</label>
                        <input type="number" name="total_sessions" min="0" step="0.5" id="totalSessions"
                            value="{{ old('total_sessions', (int)($payroll->total_sessions ?? 0)) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Tarif per Sesi (Rp)</label>
                        <input type="number" name="session_rate" min="0" step="100" id="sessionRate"
                            value="{{ old('session_rate', $payroll->total_sessions > 0 ? (int)($payroll->session_bonus / $payroll->total_sessions) : 0) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 px-4 py-2.5 text-sm transition-all">
                    </div>
                    <div>
                ...
                <div class="rounded-xl bg-gray-50 p-4 space-y-2">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="font-medium" id="previewBase">Rp {{ number_format($payroll->base_salary, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Tunjangan Lain</span>
                        <span class="font-medium" id="previewExtra">Rp {{ number_format($payroll->salary_extra, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Bonus Sesi</span>
                        <span class="font-medium text-emerald-600" id="previewSession">Rp {{ number_format($payroll->session_bonus, 0, ',', '.') }}</span>
                    </div>
                    <div class="border-t pt-2 flex justify-between text-sm">
                        <span class="text-gray-600">Total Kompensasi</span>
                        <span class="font-bold" id="previewTotal">Rp {{ number_format($payroll->total_compensation, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">BPJS Kesehatan</span>
                        <span class="font-medium text-red-600" id="previewTransport">-Rp {{ number_format($payroll->transport_allowance, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">BPJS Ketenagakerjaan</span>
                        <span class="font-medium text-red-600" id="previewDeductions">-Rp {{ number_format($payroll->deductions, 0, ',', '.') }}</span>
                    </div>
                    <div class="border-t pt-2 flex justify-between">
                        <span class="font-bold text-gray-900">Total Bersih</span>
                        <span class="font-bold text-emerald-600 text-lg" id="previewNet">Rp {{ number_format($payroll->net_salary, 0, ',', '.') }}</span>
                    </div>
                </div>
...
function updatePreview() {
    ...
    const extra = parseFloat(document.getElementById('salaryExtra').value) || 0;
...
['baseSalary', 'salaryExtra', 'totalSessions', 'sessionRate', 'transportAllowance', 'deductions'].forEach(id => {
