...
@section('page-title', 'Edit Jenis Terapi')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('therapy-types.index') }}" class="p-2 rounded-xl hover:bg-gray-100 transition-colors flex-shrink-0">
            <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <h1 class="text-xl font-bold text-gray-900">Edit Jenis Terapi</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 sm:p-8">
        <form action="{{ route('therapy-types.update', $therapyType) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Terapi <span class="text-red-500">*</span></label>
                <input type="text" name="name" required value="{{ old('name', $therapyType->name) }}"
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="price_per_session" required min="0" step="1000" value="{{ old('price_per_session', $therapyType->price_per_session) }}"
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                    class="w-full rounded-xl border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 px-4 py-2.5 text-sm transition-all">{{ old('notes', $therapyType->notes) }}</textarea>
            </div>
